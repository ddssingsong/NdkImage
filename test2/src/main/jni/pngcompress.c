...
int png_to_jpeg(const char *pngfile, const char *jpegfile, int jpegquality) {
    ...
    int ret = 0;
    png_structp png_ptr;
    png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, 0, 0,
                                     0);
    assert(png_ptr);
    ...
    if (color_type != PNG_COLOR_TYPE_RGB_ALPHA && color_type != PNG_COLOR_TYPE_RGB) {
        LOGD("input PNG must be RGB+Alpha");
        ret = 4;
        goto error_png;
    }
    if (bit_depth != 8) {
        LOGD("input bit depth must be 8bit!");
        ret = 5;
        goto error_png;
    }
    int channels = png_get_channels(png_ptr, info_ptr);
    if (channels != 4 && channels != 3) {
        LOGD("channels must be 4!");
        ret = 6;
        goto error_png;
    }
